...
we compute the similarities matrix and we get the next datsets for w2v and BERT (free-context), for sum and average as composition function,
...
NOTE: sum and average the same results???
...
lst_def_ordered <- lapply(1:nrow(df_sim), function(c){sim_one_word <- data.frame(def = names(t(df_sim[c, -c("w"), with = FALSE])[ , 1]), 
                                                                                 sim = t(df_sim[c, -c("w"), with = FALSE])[ , 1])
                                                        setDT(sim_one_word)
...
lst_sim_ordered <- lapply(1:nrow(df_sim), function(c){sim_one_word <- data.frame(def = names(t(df_sim[c, -c("w"), with = FALSE])[ , 1]), 
                                                                                 sim = t(df_sim[c, -c("w"), with = FALSE])[ , 1])
                                                        setDT(sim_one_word)
                                                        setorder(sim_one_word, -sim)
...
lst_ranking <- list(dt_def_ordered, dt_sim_ordered)

#saveRDS(lst_ranking, paste0(path_root_proj, "data/exchange/20200118-test3-wToDef/w2v_def_sum_ranking.rds"))
#saveRDS(lst_ranking, paste0(path_root_proj, "data/exchange/20200118-test3-wToDef/w2v_def_avg_ranking.rds"))
#saveRDS(lst_ranking, paste0(path_root_proj, "data/exchange/20200118-test3-wToDef/bert_def_sum_ranking.rds"))
saveRDS(lst_ranking, paste0(path_root_proj, "data/exchange/20200118-test3-wToDef/bert_def_avg_ranking.rds"))
```
