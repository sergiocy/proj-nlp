    ...
    if embeddings_model is not None:
        ...
        for w in lst_phrase:
            ...
            if w in embeddings_model.vocab:
                ...
                df_emb = np_array_as_row_of_pd_df(logger = None
                                        , np_array = embeddings_model[w] #embeddings_model.wv[w]
                                        , pd_colnames_root = root_colnames)

                if logger is not None:
                    logger.info("dimension generate embedding - {0} - {1}".format(df_emb.shape[1], w))

                lst_embed_dfs.append(df_emb)
            else:
                if logger is not None:
        ...
        if logger is not None:
            logger.info("dimension generated embeddings dataframe, {0}, for token - {1}".format(rep_vect.shape
                                                                                                , lst_phrase))
